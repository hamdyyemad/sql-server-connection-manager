...
export const updateUserRoleHandler: DatabaseHandler = async (req, pool) => {
  const { database, userId, newRoleId } = req.parsedBody;
  
  if (!database || !userId || !newRoleId) {
    return {
      success: false,
      error: 'Database name, user ID, and new role ID are required'
    };
  }
  ...
  try {
    ...
    try {
      ...
      console.log(`Updated role for user ${userId} to role ${newRoleId} in database ${sanitizedDatabase}`);
