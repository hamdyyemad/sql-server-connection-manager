...
async function authPagesMiddleware(request: NextRequest): Promise<NextResponse | null> {
  ...
  try {
    ...
    console.log(`[AuthMiddleware] is2FAEnabled: ${is2FAEnabled}, hasSetup2FA: ${hasSetup2FA}, is2FAVerified: ${is2FAVerified}, needsVerification: ${needsVerification}, secret2FAHasValue: ${secret2FAHasValue}, tempSecret2FAHasValue: ${tempSecret2FAHasValue}`);
...
async function getUserFlags(request: NextRequest) {
  ...
  try {
    ...
    // Use check2FAStatusAction to get user status
    const { check2FAStatusAction } = await import("@/backend_lib/actions/2fa");
    const result = await check2FAStatusAction();
